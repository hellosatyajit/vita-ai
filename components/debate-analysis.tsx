...
export default function DebateAnalysis({ analysis }: { analysis: DebateAnalysisType }) {
  return (
                  ...
                  <ul className="space-y-2">
                    {analysis.argument_analysis.main_arguments.map((arg, i) => (
                      <li key={i} className="flex items-start gap-2">
                        ...
                        <span className={arg.includes("FOR") ? "" : "text-red-500"}>{arg}</span>
                      </li>
                    ))}
                  </ul>
                </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {analysis.argument_analysis.logical_fallacies.map((fallacy, i) => (
                      <div key={i} className="rounded-lg border bg-red-50/50 p-3 text-sm">
                        {fallacy}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="grid gap-6 md:grid-cols-2">
                    <div className="flex flex-col items-center">
                      <div className="relative flex h-32 w-32 items-center justify-center rounded-full border-8 border-primary/20 bg-primary/10">
                        ...
                        <span className="absolute bottom-5 text-sm font-medium text-primary/80">/10</span>
                      </div>
                    ...
                    <div className="flex flex-col items-center">
                      <div className="relative flex h-32 w-32 items-center justify-center rounded-full border-8 border-blue-300/50 bg-blue-50">
                        <span className="text-4xl font-bold text-blue-600 leading-none">{analysis.rhetorical_analysis.clarity_score}</span>
                        <span className="absolute bottom-5 text-sm font-medium text-blue-600/80">/10</span>
                      </div>
                  ...
                  <div className="space-y-3">
                    <div className="rounded-lg border bg-muted/50 p-4">
                      ...
                      <div className="flex flex-wrap gap-2 text-sm">
                        {analysis.rhetorical_analysis.notable_phrases.map((phrase, i) => (
                          <p>{phrase}</p>
                        ))}
                      </div>
                    </div>
              ...
              <CardContent>
                ...
                <div className="mt-4">
                  <div className="mb-1 flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium">{analysis.overall_assessment.effectiveness_score}/10</span>
                  </div>
                  <Progress value={analysis.overall_assessment.effectiveness_score * 10} className="h-2" />
                </div>
              </CardContent>
            </Card>
          </div>
              ...
              <Card className="border-primary/20 bg-primary/5">
                ...
                <CardContent>
                  <ul className="space-y-3">
                    {analysis.improvement_areas.priority_improvements.map((item, i) => (
                      <li key={i} className="flex items-start gap-3 rounded-lg bg-primary/10 p-3">
              ...
              <Card>
                ...
                <CardContent>
                  <ul className="space-y-2 text-sm">
                    {analysis.improvement_areas.practice_suggestions.map((item, i) => (
                      <li key={i} className="flex items-start gap-2">
            ...
            <Card>
              ...
              <CardContent>
                <ul className="space-y-3 text-sm">
                  {analysis.improvement_areas.specific_examples.map((item, i) => (
                    <li key={i} className="flex items-start gap-3 rounded-lg border bg-amber-50/50 p-3">
            ...
            <CardContent className="flex flex-1 flex-col items-center justify-center">
              <div className="flex h-32 w-32 items-center justify-center rounded-full border-8 border-amber-200 bg-amber-50">
                <div className="text-center">
                  <div className="text-5xl font-bold text-amber-600">{analysis.overall_assessment.effectiveness_score}</div>
                  <div className="text-sm font-medium text-amber-600/80">out of 10</div>
                </div>
              </div>
